...
import LUcalculate from '../calculateAGB/LUcalculete';
...
function Ludecomposition() {
  ...
  const sendTocal = async (e) => {
      ...
      let a = await LUcalculate(object);
      setanser(a);
      setmatrix(matrixA)
  };
  ...
  const savetodatabase = async(e) => {
    e.preventDefault()
    const typeform = {...object,type : "Linear",anser : anser.anserX, subtype : "LU Deconposit"}
    const dataobject = {
      dataobject : typeform,
      type : "Linear"
    }
  ...
  return (
      <div>
          <div className='text-center text-3xl'>
              <h1 className='text-white pt-10 pb-5'>Linear Algebra: LU decomposition</h1>
              ...
              <div className='mt-10 bg-slate-500 rounded-md text-white w-[70%]'>
                  ...
                  {anser && anser.anserX && (
                      ...
                      <div>
                          ...
                          <BlockMath math={`\Matrix \\ L
                            \\begin{bmatrix}
                            \\begin{matrix}
                            ${anser.anserL.map((row,rowindex)=>(
                              Array.isArray(row) ? row.map((value,colindex) =>{
                                return (rowindex === 0 && colindex === 0) ? `${value}` : value
                              }).join(' & ') : ''
                            )).join(' \\\\\n ')}
                            \\end{matrix}
                            \\end{bmatrix}`} />
                            <BlockMath math={`\Matrix \\  U
                            \\begin{bmatrix}
                            \\begin{matrix}
                            ${anser.anserU.map((row,rowindex)=>(
                              Array.isArray(row) ? row.map((data,colindex) =>{
                                return (rowindex === 0 && colindex === 0) ? `${data}` : data
                              }).join(' & ') : ''
                            )).join(' \\\\\n ')}
                            \\end{matrix}
                            \\end{bmatrix}`} />
